...
function buildReferenceEntries(slug, config) {
  if (slug === "doctor") return DOCTOR_REFERENCE_ENTRIES;

  return config.specialties.slice(0, 4).map((specialty, index) => ({
    name: personName(index + 20),
    specialty,
    distance: Number((0.5 + index * 0.3).toFixed(1)),
    rating: Number((4.6 + (index % 4) * 0.1).toFixed(1)),
    startingPrice: 800 + index * 350,
    serviceModes: index % 2 === 0
...
export function getCategoryListingProviders(slug, categoryId, activeFilter = "All") {
  ...
  const categoryProviders = mockProviders.filter((provider) => provider.categoryId === categoryId);
  const sourceProviders = categoryProviders.length >= 6
    ? categoryProviders
    : mockProviders;

  return listingNames.map((name, index) => {
    ...
    const specialty = activeFilter && activeFilter !== "All"
      ? activeFilter
    ...
    return {
      ...
      distance: referenceEntry?.distance ?? Number((0.4 + (index % 10) * 0.2).toFixed(1)),
      startingPrice: referenceEntry?.startingPrice ?? 800 + (index % 12) * 150,
      serviceModes: buildServiceModes(index, referenceEntry),
    };
  }).filter((provider) => activeFilter === "All" || provider.specialty === activeFilter);
}
