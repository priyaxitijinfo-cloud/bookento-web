...
import {
  Mic, MicOff, PhoneOff, SwitchCamera, Video, VideoOff,
} from "lucide-react";
    ...
    <div className="relative h-dvh overflow-hidden bg-black">
      ...
      <div className="safe-top absolute inset-x-0 top-0 z-10 flex items-center justify-between p-4">
        <div className="flex items-center gap-3">
          <Avatar src={provider.avatar} name={provider.businessName} size="sm" className="border-2 border-white" />
          <div>
      ...
      {/* Local video (self) */}
      <div className="absolute right-4 top-20 z-10 h-36 w-28 overflow-hidden rounded-2xl border-2 border-white/30 shadow-lg">
        {videoOff ? (
          ...
          <Image
            src="https://picsum.photos/seed/self-video/200/300"
            alt="You"
            fill
            className="object-cover"
          />
      ...
      {!connected && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-black/60">
          <Avatar src={provider.avatar} name={provider.businessName} size="xl" className="size-24 border-4 border-white/30" />
          <p className="mt-4 text-lg font-semibold text-white">Calling {provider.businessName}...</p>
      ...
      <div className="safe-bottom absolute inset-x-0 bottom-0 z-10 flex items-center justify-center gap-4 p-8">
        <Button
          variant="secondary"
          size="icon"
          className="size-14 rounded-full bg-white/20 text-white backdrop-blur hover:bg-white/30"
        ...
        <Link href={ROUTES.CHATS}>
          <Button variant="destructive" size="icon" className="size-16 rounded-full">
