  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center md:items-center md:p-6">
      <button
        ...
        aria-label="Close"
      />
      <div className="bg-background relative flex max-h-[90vh] w-full max-w-2xl flex-col rounded-t-xl shadow-2xl md:rounded-xl">
        <div className="flex items-center justify-between px-5 pt-2.5 md:border-b md:py-4">
        ...
        <div className="px-5 py-3 md:border-b">
          <div className="relative">
            <ResponsiveSearchIcon className="text-muted-foreground pointer-events-none absolute top-1/2 left-3 size-5 -translate-y-1/2 md:size-4" />
            <input
              type="search"
              value={query}
              onChange={(event) => setQuery(event.target.value)}
              placeholder="Search services"
              className="border-border ring-primary/20 focus:bg-background w-full rounded-full border bg-[#FAFBFD] py-2.5 pr-4 pl-11 text-sm outline-none focus:ring-2 md:rounded-lg md:pl-10"
            />
          </div>
        </div>

        <div className="-mt-1.5 flex-1 space-y-2.5 overflow-y-auto px-5 py-4 md:mt-0">
